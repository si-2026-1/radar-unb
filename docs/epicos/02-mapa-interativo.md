...
**Visualizar mapa interativo:** Como usuário, quero visualizar um mapa interativo e responsivo do campus da UnB, para ter uma visão geral das ocorrências cadastradas.
...
**Visualizar detalhes ao clicar no marcador:** Como usuário, quero clicar em um marcador georreferenciado no mapa, para visualizar em um card ou modal os detalhes do evento ou problema (título, descrição, foto).
...
**Aplicar filtros no mapa:** Como usuário, quero aplicar filtros (por tipo, data e localização) no mapa, para focar apenas em eventos acadêmicos ou apenas em problemas de segurança recentes.
...
**Diferenciar tipos de marcadores:** Como usuário, quero visualizar as ocorrências e eventos como marcadores distintos no mapa, para diferenciar rapidamente o tipo de cada registro sem precisar clicar no ícone.
...
**Agrupar marcadores próximos:** Como usuário, quero que marcadores muito próximos sejam agrupados quando o zoom estiver distante, para evitar a poluição visual e manter o mapa legível.
...
**Ver posição atual no mapa:** Como usuário, quero ver a minha posição atual no mapa, para me orientar em relação aos problemas e eventos ao meu redor.
...
**Carregar dados dinamicamente:** Como usuário, quero que o mapa carregue os dados dinamicamente conforme mudo de região, para garantir o desempenho e poupar consumo de dados.
...
**Limitar navegação ao campus:** Como usuário, quero que o mapa não me permita navegar para fora do campus da UnB, para manter o foco na área de cobertura da plataforma. *(RF-MAP-06)*
